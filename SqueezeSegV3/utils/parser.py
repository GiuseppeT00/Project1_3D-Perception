...
class SemanticKitti(Dataset):
  ...
  def __init__(self, root,    # directory where data is
    ...
    print(self.nclasses, flush=True)
    # sanity checks

    # make sure directory exists
    if os.path.isdir(self.root):
      print("Sequences folder exists! Using sequences from %s" % self.root, flush=True)
    else:
    ...
    for seq in self.sequences:
      ...
      print("parsing seq {}".format(seq), flush=True)
  ...
  def map(label, mapdict):
    ...
    for key, data in mapdict.items():
      try:
        ...
        print("Wrong key ", key, flush=True)
    # do the mapping
    return lut[label]
