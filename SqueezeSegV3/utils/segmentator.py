...
class Segmentator(nn.Module):
  def __init__(self, ARCH, nclasses, path=None, path_append="", strict=False):
    ...
    bboneModule = imp.load_source("bboneModule", 'utils/' + self.ARCH["backbone"]["name"] + '.py')
    ...
    decoderModule = imp.load_source("decoderModule", 'utils/' + self.ARCH["decoder"]["name"] + '.py')
    ...
    print("Total number of parameters: ", weights_total, flush=True)
    print("Total number of parameters requires_grad: ", weights_grad, flush=True)
    ...
    print("Param encoder ", weights_enc, flush=True)
    print("Param decoder ", weights_dec, flush=True)
    print("Param head ", weights_head, flush=True)
    if self.CRF:
      weights_crf = sum(p.numel() for p in self.CRF.parameters())
      print("Param CRF ", weights_crf, flush=True)
    ...
    if path is not None:
      ...
      try:
        ...
        print("Successfully loaded model backbone weights", flush=True)
      except Exception as e:
        print(flush=True)
        print("Couldn't load backbone, using random weights. Error: ", e, flush=True)
        if strict:
          print("I'm in strict mode and failure to load weights blows me up :)", flush=True)
          raise e
      ...
      try:
        ...
        print("Successfully loaded model decoder weights", flush=True)
      except Exception as e:
        print("Couldn't load decoder, using random weights. Error: ", e, flush=True)
        if strict:
          print("I'm in strict mode and failure to load weights blows me up :)", flush=True)
          raise e
      ...
      # try head
      try:
        print(path_append+'./segmentation_head1', flush=True)
        w_dict = torch.load(path + "/segmentation_head1",
                            map_location=lambda storage, loc: storage)
        self.head1.load_state_dict(w_dict, strict=True)
        print("Successfully loaded model head weights", flush=True)
      except Exception as e:
        print("Couldn't load head, using random weights. Error: ", e, flush=True)
        if strict:
          print("I'm in strict mode and failure to load weights blows me up :)", flush=True)
          raise e
      try:
      ...
      print("No path to pretrained, using random init.", flush=True)
