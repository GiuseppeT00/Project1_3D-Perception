...
class Trainer():
  def __init__(self, ARCH, DATA, datadir, logdir, path=None):
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
      ...
      print('Found more GPUs, setting cuda to cuda:0', flush=True)
    '''

    # get the data
    parserModule = imp.load_source("parserModule", 'utils/parser.py')
    ...
    for x_cl, w in enumerate(self.loss_w):  # ignore the ones necessary to ignore
      if DATA["learning_ignore"][x_cl]:
        # don't weigh
        self.loss_w[x_cl] = 0
    print("Loss weights from content: ", self.loss_w.data, flush=True)
    ...
    print("Training in device: ", self.device, flush=True)
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
      ...
      print("DataParallel correctly applied to model", flush=True)
      self.model = convert_model(self.model).cuda()  # sync batchnorm
      print("Model BN's successfully converted to SyncBN's", flush=True)
      self.model_single = self.model.module.to('cuda:0') # single model to get weight names
      self.multi_gpu = True
    ...
    if self.n_gpus > 1:
      ...
      print("DataParallel correctly applied to criterion", flush=True)
    ...
    if self.ARCH["head"]["train"]:
      ...
      self.train_dicts.append({'params': self.model_single.head1.parameters()})
      self.train_dicts.append({'params': self.model_single.head2.parameters()})
      self.train_dicts.append({'params': self.model_single.head3.parameters()})
      self.train_dicts.append({'params': self.model_single.head4.parameters()})    
      self.train_dicts.append({'params': self.model_single.head5.parameters()})
  ...
  def train(self):
    ...
    for i, w in enumerate(self.loss_w):
      if w < 1e-10:
        ...
        print("Ignoring class ", i, " in IoU evaluation", flush=True)
    ...
    for epoch in range(self.ARCH["train"]["max_epochs"]):
      ...
      if iou > best_train_iou:
        print("Best mean iou in training set so far, save model!", flush=True)
        best_train_iou = iou
      ...
      if epoch % self.ARCH["train"]["report_epoch"] == 0:
        ...
        # remember best iou and save checkpoint
        if iou > best_val_iou:
          print("Best mean iou in validation so far, save model!", flush=True)
          print("*" * 80)
          best_val_iou = iou
      ...
      if i % self.ARCH["train"]["report_batch"] == 0:
        print('Lr: {lr:.3e} | '
              ...
              'IoU {iou.val:.3f} ({iou.avg:.3f})'.format(
                  ...
                  umean=update_mean, ustd=update_std), flush=True)
