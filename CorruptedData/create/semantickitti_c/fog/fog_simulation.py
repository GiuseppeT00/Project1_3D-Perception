...
def parse_arguments():
    ...
    parser.add_argument('-b', '--beta', help='backscattering coefficient', type=float,
                        default=0.008) 
    parser.add_argument('-s', '--sequences', help='sequences to generate', type=str, default='08', nargs='+')
...
if __name__ == '__main__':
    ...
    for seq in args.sequences:
        print(f"{'*' * 30}\nGenerating sequence {seq}...\n{'*' * 30}")
        all_files = []
        all_files += absoluteFilePaths('/'.join([src_folder, str(seq).zfill(2), 'velodyne']))
        all_files.sort()
        ...
        def _map(i: int) -> None:
            ...
            lidar_save_path = os.path.join(dst_folder,str(seq).zfill(2),'velodyne', all_files[i].split('/')[-1])
            ...
            label_save_path1 =  os.path.join(dst_folder,str(seq).zfill(2),'labels', all_files[i].split('/')[-1].replace('bin', 'label'))
            if not os.path.exists(os.path.dirname(label_save_path1)):
                os.makedirs(os.path.dirname(label_save_path1), exist_ok=True)
